...
@app.route('/getRestaurantOrders', methods=['POST'])
@cross_origin()
def getRestaurantOrders():
    ...
    if (decoded_JSON and restaurantId == decoded_JSON['sub']):
        ...
        my_cursor = my_connect.cursor(dictionary=True)
        my_cursor.execute("SELECT * FROM orders where restaurantName = %s", (restaurantName,))
        orders = my_cursor.fetchall()
        data = {"orders": orders}
        if orders:
            status = 200
    ...
    if (decoded_JSON and customerId == decoded_JSON['sub']):
        ...
        try:
            ...
            for foodItem in foodItems:
                my_cursor.execute("INSERT INTO order_foodItems(orderId,foodItemId,quantity,foodItemName) values (%s,%s,%s,%s)",
                                  (order_id, foodItem['id'], foodItem['quantity'], foodItem["foodItemName"]))
